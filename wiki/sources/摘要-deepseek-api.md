...
> [!important] 一句话摘要
> DeepSeek API 同时兼容 OpenAI 和 Anthropic 两种 API 格式，通过简单切换 base_url 即可用现有 SDK 调用 DeepSeek 模型；但 Anthropic 兼容层存在显著能力缺口（无多模态、无 MCP、无 prompt cache）。
...
- [[DeepSeek]] — 公司实体
- [[API兼容性策略]] — 双兼容策略概念详解
- [[AgentSkills]] — Skills 与 API 格式同为 Anthropic 推动的开放标准
