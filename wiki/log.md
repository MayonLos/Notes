...
- 保留 `raw/09-archive/锁存器与触发器.pdf`（非图片）
- 更新 `sources: []` 的笔记（8个）：
  - `逻辑函数化简.md`、`逻辑门电路.md`、`组合逻辑电路.md`、`锁存器和触发器.md`
  - `计数制与编码.md`、`逻辑代数.md`、`原码反码补码.md`
  - `摘要-数字电路课堂笔记.md`
- `摘要-锁存器与触发器.md` sources 路径修正为正确的 archive 位置

## [2026-06-22] cleanup | 数字电路笔记整理——删除冗余内容

- `组合逻辑电路.md`：删除「§一 基本逻辑门」整节（~45行），含逻辑门表、布尔代数基础、卡诺图化简小节，全部与 [[逻辑代数]] [[逻辑函数化简]] 重复；改为一行引用
- `逻辑代数.md`：删除「§4.1 建立步骤」五步流程，已由 [[逻辑函数化简]] 完整覆盖；原 4.2 升格为 4.1
- `计数制与编码.md`：16行完整二进制↔十六进制对照表压缩为5行关键行 + 规律说明

## [2026-06-22] update | Laplace变换 — 新增留数定理章节

- 在 `wiki/concepts/Laplace变换.md` 扩充「拉普拉斯逆变换」板块
- 新增「留数定理求逆变换」完整章节，含三种极点情形
  - 情形一：单极点（互不相同实极点），例：$\frac{2}{(s+1)(s+3)}$
  - 情形二：重极点（逐阶求导系数），例：$\frac{1}{s(s+1)^2}$
  - 情形三：共轭复极点（配凑法+留数法双讲），例：$\frac{5}{s(s^2+4s+5)}$
- 附极点位置与时域响应对应关系速查表
