...
## [2026-05-05] query | SR 锁存器逻辑代数推导与记忆原理
- **输出**：从 NAND 门交叉耦合方程推导 SR 锁存器特征方程 $Q^{n+1}=S_d+\overline{R_d}\cdot Q^n$，阐述双稳态记忆原理
- **后续操作**：
  - 填充 [[concepts/digital/锁存器和触发器]] — 新建完整概念页（特征方程推导、状态真值表、记忆/双稳态原理、各类触发器概览）
  - 更新 [[index|Wiki 索引]] — 注册新页面、修复学习路线中「③时序逻辑（待填充）」占位
- **冲突**：无
...
## [2026-05-05] ingest | 数字电路课堂笔记（52页手写笔记批量摄入）

- **变更**：
  - 新增 [[sources/摘要-数字电路课堂笔记]] — 52页笔记总源摘要页（四章全貌，2026-03-02→04-29）
  - 新增 [[concepts/digital/逻辑门电路]] — DTL/TTL/CMOS 三代工艺、OC/OD/TSL/TG 门
  - 新增 [[comparisons/TTL-vs-CMOS]] — 逻辑器件选型多维度对比（功耗/速度/噪声容限/接口）
  - 更新 [[concepts/digital/组合逻辑电路]] — 补充 MSI 设计法（MUX/译码器实现任意函数）、加法器综合应用（可控加减法器、码制转换）
  - 更新 [[concepts/digital/锁存器和触发器]] — 补全同步SR触发器/主从结构/JK触发器/T触发器/波形图，消除 `[!todo]` 占位
  - 更新 [[concepts/digital/逻辑函数化简]] — 嵌入 4变量含无关项卡诺图 SVG
  - 更新 [[index|Wiki 索引]] — 新增 4 个概念条目 + 对比条目 + 来源条目，学习路线③占位修复
  - 新增图片：`sr_latch_nand.svg`、`jk_ff_master_slave.svg`、`kmap_4var_dc.svg`
  - 归档：52 个源文件 → `raw/09-archive/2026-05-05-数字电路课堂笔记/`
- **冲突**：无
- **来源**：`raw/04-notes/`（52 张手写笔记照片，IMG_20260302 至 IMG_20260429）
